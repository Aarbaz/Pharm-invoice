      ...
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          <p data-placement="top" data-toggle="tooltip">
            <a class="btn btn-primary btn-sm" href="<?php echo base_url('/index.php/Material/');?>">Go back to Material list</a>
          </p><hr />           
          <?php            
            echo form_open('Material/add_new', 'class="form-horizontal" id="add_material_form"');
          ?>
            <div class="form-group">
              <table class="table table-bordered">
                ...
                <tbody>
                  <tr class="row_one">                    
                    <td><input type="text" name="mat_name[]" class="form-control" style="width: 100%;"></td>
              ...
              <div class="col-sm-4">                  
                <select name="vendorName" id="vendorName" style="width: 60%; display: inline;" class="form-control">
                  ...
                  <?php foreach ($vendors->result() as $row){
                    echo '<option value="'.$row->vendor_id.'" '.set_select('vendorName',$row->vendor_id).'>'.$row->vendor_name.'</option>';
                  } ?>
                </select>
              </div>
            ...
            <div class="form-group">
              <div class="col-sm-5">              
                <?php echo form_submit('add_material','Add Material','class="btn btn-success"'); ?>
              </div>
              <div class="col-sm-6"> 
                <?php
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    var cust_list = <?php echo json_encode($vendors->result()); ?>;
    $("#vendorName").on('change', function(){
      var cust_id = $("#vendorName option:selected").val();
      for (var key = 0; key < cust_list.length; key++) 
      {
        if( cust_list[key].vendor_id == cust_id )
        {            
          $("#last_bal").val(cust_list[key].debit_balance);
        }
      }
    });
